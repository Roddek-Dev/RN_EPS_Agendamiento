  ...
  const PatientCard = ({ patient }: { patient: (typeof patients)[0] }) => (
    <View style={globalStyles.listCard}>
      ...
      <View style={{ flex: 1, marginLeft: 12 }}>
        <View style={[globalStyles.row, { justifyContent: "space-between", marginBottom: 4 }]}>
          <Text style={globalStyles.itemTitle}>{patient.name}</Text>
          <StatusBadge status={patient.status} />
        </View>

        <Text style={[globalStyles.caption, { marginBottom: 8 }]}>{patient.age} años</Text>

        <ContactInfo phone={patient.phone} email={patient.email} />

        <Text style={[globalStyles.caption, { color: colors.primary, fontWeight: "500" }]}>
      ...
      <View style={globalStyles.listItemActions}>
        <TouchableOpacity
          ...
          onPress={() => router.push(`/(main)/cruds/patients/${patient.id}`)}
        >
          <Eye color={colors.infoText} size={16} />
        </TouchableOpacity>
        <TouchableOpacity
          style={[globalStyles.actionButton, { backgroundColor: colors.success }]}
          onPress={() => router.push(`/(main)/cruds/patients/edit/${patient.id}`)}
        >
    ...
    <SafeAreaView style={globalStyles.container}>
      <View style={globalStyles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push("/(main)/cruds/patients/create")}
        >
