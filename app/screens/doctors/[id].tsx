...
import { Briefcase, Star, Phone, Mail, Calendar, Users, Activity } from "lucide-react-native"
  ...
  return (
    <SafeAreaView style={globalStyles.container}>
      <ProfileHeader
        ...
        onBack={() => router.back()}
        onEdit={() => router.push(`/(main)/cruds/doctors/edit/${id}`)}
      />

      <ScrollView contentContainerStyle={globalStyles.content}>
        {/* Perfil principal */}
        <View style={globalStyles.profileSection}>
          <Image source={{ uri: doctor.avatar }} style={globalStyles.profileAvatar} />
          <Text style={[globalStyles.title, { textAlign: "center", marginBottom: spacing.md }]}>{doctor.name}</Text>
          <Text style={[globalStyles.subtitle, { textAlign: "center", marginBottom: spacing.xl }]}>
            {doctor.specialty}
          </Text>

          <View style={globalStyles.statContainer}>
            <StatItem icon={<Star color="#fbbf24" size={24} fill="#fbbf24" />} value={doctor.rating} label="Rating" />
            <StatItem
        ...
        <View style={globalStyles.card}>
          ...
          <DetailRow icon={Briefcase} label="Especialidad" value={doctor.specialty} />
          <DetailRow icon={Activity} label="Experiencia" value={doctor.experience} />
        ...
        <View style={globalStyles.card}>
          ...
          <DetailRow icon={Users} label="Pacientes atendidos" value={`${doctor.patients} pacientes`} />
          <DetailRow icon={Calendar} label="Citas completadas" value={`${doctor.completedAppointments} citas`} />
          <DetailRow
            icon={Activity}
            label="Citas pendientes"
            value={`${doctor.appointments - doctor.completedAppointments} citas`}
          />
        </View>
        ...
        <MetadataSection
          ...
          items={[
            ...
            { label: "Estado", value: doctor.status === "available" ? "Disponible" : "No Disponible" },
          ]}
        />
      </ScrollView>
    </SafeAreaView>
